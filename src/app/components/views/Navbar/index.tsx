...
import { NavbarArray,NavbarItemType } from "@/app/components/utils/NavBarArraysAndTypes";
...
<div className="coverdiv sticky top-0 bg-white/70 backdrop-blur-sm  z-50">
    ...
    <div className='1 w-36 flex-shrink-0'>
        <Image className="" alt="logo" height={500} width={500} src={"/Logo.webp"} />
    </div>
    ...
    <ul className=" 2 flex space-x-5 font-light text-lg text-purple-950 mr-32">
      {NavbarArray.map((item :NavbarItemType ,index:number )=>(
             <li key={index} className="flex items-center relative rounded-sm px-3 py-1 hover:bg-gray-100 cursor-pointer group ">
              ...
              {/* ternary opreator */}
              {item.isDropDown ? <AiOutlineDown className="  mt-1 -rotate-90 group-hover:rotate-0 duration-300"  size={12}/> :""}
              {/* conditional rendering */}
              {item.isDropDown && <div className="invisible group-hover:visible absolute top-8 left-0 mr-1  p-5 rounded-md bg-gray-100 font-light min-w-[7.8rem] my-2"><DropDown item={item}/></div>}
    ...
    <div className=" 3 border flex items-center px-3 rounded-md mr-10 ">
      ...
      <input type="text" className="pl-2 pr-4 py-[0.10] outline-none"
    ...
    <div className="4 flex-shrink-0 relative w-11 h-11 bg-gray-200 rounded-full flex items-center justify-center hover:cursor-pointer">
     <div className="4child absolute top-1 right-1 bg-red-400  font-light rounded-full w-4 h-4 flex justify-center items-center text-xs">{cartItemNumber}</div> 
    <BsCart2 size={24}/>
     </div>
    
    </div> 
    <div className='parent' onClick={()=>setNavbarOpen(!isNavbarOpen)}> {/* Hamburger  */}
    {
        isNavbarOpen ?
      ...
      <div className="w-full px-6 py-4 bg-gray-100">
          {
              NavbarArray.map((item: NavbarItemType, index: number) => {
                  return (
                      <Expand key={index} Dataitem={item} />
                  )
              })
          }
      </div>
